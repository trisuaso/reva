...
struct Foo;"##
        ...
        let expected = format!(
            r#"impl ::reva::Template for Foo {{
    fn render_into(&self, writer: &mut (impl ::std::fmt::Write + ?Sized)) -> ::reva::Result<()> {{
        {new_expected}
        ::reva::Result::Ok(())
    }}
...
impl ::std::fmt::Display for Foo {{
    ...
    ::std::write!(
        writer,
        "{expr0}",
        expr0 = &::reva::MarkupDisplay::new_unsafe(&(query), ::reva::Text),
    )?;
}"#,
    );
    ...
    ::std::write!(
        writer,
        "{expr0}",
        expr0 = &::reva::MarkupDisplay::new_unsafe(&(s), ::reva::Text),
    )?;
}"#,
    );
